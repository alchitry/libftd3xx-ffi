...
static void write_test(FT_HANDLE handle)
{
	...
	for (i  = 0; i < QUEUE_LEN; i++) 
	{
		...
		ftStatus = FT_WritePipeAsync(handle, channel,
			buf[i].get(), BUFFER_LEN, &count, &pOverlapped[i]);
		if((ftStatus != FT_IO_PENDING)&&(ftStatus != FT_OK))
		{
			printf("FT_WritePipeAsync ERROR return status =%d  writeCount=%d\n",ftStatus, count);
		}
	}
	...
	{	
		...
		{
			printf("FT_GetOverlappedResult ERROR return status =%d  writeCount=%d\n",ftStatus, count);
			do_exit = true;
			break;
		}
...
static void read_test(FT_HANDLE handle)
{
	...
	for (i  = 0; i < QUEUE_LEN; i++) 
	{
		...
		ftStatus = FT_ReadPipeAsync(handle, channel,
			buf[i].get(), BUFFER_LEN, &count, &pOverlapped[i]);
		if((ftStatus != FT_IO_PENDING)&& (ftStatus != FT_OK))
		{
			printf("FT_ReadPipeAsync ERROR return status =%d  readCount=%d\n",ftStatus, count);
		}
	}
	...
	{	
		...
		if(FT_OK != ftStatus)
		{
			printf("FT_GetOverlappedResult ERROR return status =%d  readCount=%d\n",ftStatus, count);
			do_exit = true;
			break;
		}
