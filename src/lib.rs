...
/// Workaround FT_STATUS and _FT_STATUS setup to make it better in Rust oppose to C enums limitations.
pub type FT_Status = _FT_STATUS;
        ...
        impl TryFrom<$t> for FT_Status {
            ...
            fn try_from(v: $t) -> Result<Self, Self::Error> {
                match v {
                    ...
                    x if x == FT_Status::FT_INVALID_HANDLE as $t => {
                        Ok(FT_Status::FT_INVALID_HANDLE)
                    }
                    x if x == FT_Status::FT_DEVICE_NOT_FOUND as $t => {
                        Ok(FT_Status::FT_DEVICE_NOT_FOUND)
                    }
                    x if x == FT_Status::FT_DEVICE_NOT_OPENED as $t => {
                        Ok(FT_Status::FT_DEVICE_NOT_OPENED)
                    }
                    x if x == FT_Status::FT_IO_ERROR as $t => Ok(FT_Status::FT_IO_ERROR),
                    x if x == FT_Status::FT_INSUFFICIENT_RESOURCES as $t => {
                        Ok(FT_Status::FT_INSUFFICIENT_RESOURCES)
                    }
                    x if x == FT_Status::FT_INVALID_PARAMETER as $t => {
                        Ok(FT_Status::FT_INVALID_PARAMETER)
                    }
                    x if x == FT_Status::FT_INVALID_BAUD_RATE as $t => {
                        Ok(FT_Status::FT_INVALID_BAUD_RATE)
                    }
                    x if x == FT_Status::FT_DEVICE_NOT_OPENED_FOR_ERASE as $t => {
                        Ok(FT_Status::FT_DEVICE_NOT_OPENED_FOR_ERASE)
                    }
                    ...
                    x if x == FT_Status::FT_OPERATION_ABORTED as $t => {
                        Ok(FT_Status::FT_OPERATION_ABORTED)
                    }
                    x if x == FT_Status::FT_RESERVED_PIPE as $t => Ok(FT_Status::FT_RESERVED_PIPE),
                    x if x == FT_Status::FT_INVALID_CONTROL_REQUEST_DIRECTION as $t => {
                        Ok(FT_Status::FT_INVALID_CONTROL_REQUEST_DIRECTION)
                    }
                    x if x == FT_Status::FT_INVALID_CONTROL_REQUEST_TYPE as $t => {
                        Ok(FT_Status::FT_INVALID_CONTROL_REQUEST_TYPE)
                    }
                    ...
                    x if x == FT_Status::FT_NO_SYSTEM_RESOURCES as $t => {
                        Ok(FT_Status::FT_NO_SYSTEM_RESOURCES)
                    }
                    x if x == FT_Status::FT_DEVICE_LIST_NOT_READY as $t => {
                        Ok(FT_Status::FT_DEVICE_LIST_NOT_READY)
                    }
                    x if x == FT_Status::FT_DEVICE_NOT_CONNECTED as $t => {
                        Ok(FT_Status::FT_DEVICE_NOT_CONNECTED)
                    }
                    x if x == FT_Status::FT_INCORRECT_DEVICE_PATH as $t => {
                        Ok(FT_Status::FT_INCORRECT_DEVICE_PATH)
                    }
